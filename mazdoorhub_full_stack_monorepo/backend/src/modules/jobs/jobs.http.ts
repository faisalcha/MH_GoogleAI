...
jobsRouter.get('/', async (req,res)=>{ const lat=Number(req.query.lat), lng=Number(req.query.lng), radius=Number(req.query.radius||5000);
  if(Number.isNaN(lat)||Number.isNaN(lng)) return res.status(400).json({error:'lat,lng required'}); const c=await pgc();
  try{ const { rows } = await c.query(`SELECT id, employer_user_id, category, price, description, created_at, ST_Y(geom::geometry) AS lat, ST_X(geom::geometry) AS lng
      FROM jobs WHERE ST_DWithin(geom::geography, ST_SetSRID(ST_MakePoint($1,$2),4326)::geography, $3) ORDER BY created_at DESC LIMIT 100`, [lng,lat,radius]);
